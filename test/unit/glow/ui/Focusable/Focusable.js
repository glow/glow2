(function() {
	var frameControlsFocus = false,
		focusTest = glow('#canHaveFocusTest').on('focus', function() {
			frameControlsFocus = true;
		});
	
	focusTest[0].focus();
	focusTest.destroy();
	
	// this can happen in browsers like Firefox when the window doesn't have focus
	if (!frameControlsFocus) {
		module('glow.ui.Focusable');
		
		test('skipping all tests', 1, function() {
			ok(true, 'Skipping all tests, no elements can receive focus, ensure the window has focus when tests run');
		});
